...
8. **Decision routing**: situations are matched against the factors of your standing decisions. Today's coverage is recorded as evidence per decision, so each run updates an accumulating record rather than producing a one-off briefing. Inspect with `insightweaver decisions show <id>`.
9. **Delivery**: The brief is rendered to the terminal; pass `--save PATH` to also write a markdown copy
