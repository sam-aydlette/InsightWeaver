...
from src.context.semantic_memory import SemanticMemory  # noqa: E402
from src.database.connection import get_db  # noqa: E402
from src.database.models import Article, NarrativeSynthesis  # noqa: E402
...
async def test_semantic_memory():
    ...
    with get_db() as session:
        ...
        synthesis = session.query(NarrativeSynthesis).order_by(NarrativeSynthesis.id.desc()).first()
        ...
        print("-" * 60)
        facts = await memory.extract_facts_from_synthesis(synthesis.synthesis_data, synthesis.id)
        ...
        if facts:
            ...
            for i, sample in enumerate(facts[:3], 1):
                ...
                print(
                    f"      Object: {sample.object[:60]}..."
                    if len(sample.object) > 60
                    else f"      Object: {sample.object}"
                )
        ...
        recent_facts = session.query(MemoryFact).order_by(MemoryFact.id.desc()).limit(3).all()

        if recent_facts:
            print("\n   Most recent stored facts:")
            for fact in recent_facts:
                print(
                    f"   - {fact.subject} {fact.predicate} {fact.object[:40]}..."
                    if len(fact.object) > 40
                    else f"   - {fact.subject} {fact.predicate} {fact.object}"
                )
        print()
        ...
        if retrieved_facts:
            ...
            for i, sample in enumerate(retrieved_facts[:3], 1):
                print(
                    f"   {i}. {sample.subject} {sample.predicate} {sample.object[:40]}..."
                    if len(sample.object) > 40
                    else f"   {i}. {sample.subject} {sample.predicate} {sample.object}"
                )
