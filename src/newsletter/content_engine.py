...
class NewsletterContentEngine:
    ...
    async def generate_intelligence_report(
        ...
        print(
            f"📊 Generating intelligence report: {start_date.strftime('%Y-%m-%d %H:%M')} to {end_date.strftime('%Y-%m-%d %H:%M')} ({duration_hours:.1f}h)"
        )
        ...
        if synthesis_id is not None:
            with get_db() as session:
                synthesis = (
                    ...
                    .filter(NarrativeSynthesis.id == synthesis_id)
                    .first()
                )
                ...
                if synthesis and synthesis.synthesis_data:
                    ...
                    print(
                        f"⚠️  Warning: Synthesis ID {synthesis_id} not found in database, querying for existing synthesis..."
                    )
        ...
        with get_db() as session:
            ...
            for s in all_syntheses:
                if (
                    s.synthesis_data
                    and "metadata" in s.synthesis_data
                    and "citation_map" in s.synthesis_data.get("metadata", {})
                ):
                    synthesis = s
                    print(
                        f"✨ Using existing trust-verified synthesis with citations from {s.generated_at}"
                    )
                    break
        ...
        if use_trust_verification:
            ...
            synthesis_result = await synthesizer.synthesize_with_trust_verification(
                hours=synthesis_hours, max_articles=max_articles, max_retries=3
            )
        else:
        ...
        with get_db() as session:
            synthesis = session.query(NarrativeSynthesis).get(synthesis_result["synthesis_id"])

            if synthesis is None:
                return self._generate_empty_report(start_date, end_date, duration_hours)

            return self._format_synthesis_report(synthesis, start_date, end_date, duration_hours)

    def _format_synthesis_report(
        self, synthesis, start_date: datetime, end_date: datetime, duration_hours: float
    ) -> dict[str, Any]:
    ...
    def _generate_empty_report(
        self, start_date: datetime, end_date: datetime, duration_hours: float
    ) -> dict[str, Any]:
