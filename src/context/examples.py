...
def format_examples_for_prompt(examples: list[dict[str, Any]]) -> str:
    ...
    for i, example in enumerate(examples, 1):
        ...
        formatted += (
            "- Trends organized geographically (local → state → national → global → niche field)\n"
        )
        formatted += "- Trends formatted as observable patterns with direction and quantifiers\n"
        formatted += (
            "- Events prioritized by impact level with clear timeframes and recommended actions\n"
        )
