...
class ContextCurator:
    ...
    async def curate_for_narrative_synthesis(
        ...
        with profile("CONTEXT_CURATION_TOTAL"):
            with get_db() as session:
                ...
                # Get historical context from prior syntheses
    ...
    def _get_synthesis_instructions(self) -> str:
        """Get instructions for narrative synthesis with user context."""
        if self.user_profile:
            ...
            professional_domains = ", ".join(professional.get("professional_domains", []))
        else:
            location_str = "not specified"
            professional_domains = "not specified"

        return f"""## User Context

Location: {location_str}
Professional domains: {professional_domains}

Analyze the provided articles and generate a structured intelligence brief.
Use the user's location and professional context to assess relevance and implications."""
