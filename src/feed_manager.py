...
class FeedManager:
    ...
    def load_feeds_to_database(self) -> tuple[int, int, int]:
        ...
        with get_db() as db:
            for feed_config in self.configured_feeds:
                existing_feed = db.query(RSSFeed).filter(RSSFeed.url == feed_config["url"]).first()
            ...
            total_count = db.query(RSSFeed).filter(RSSFeed.is_active.is_(True)).count()

        logger.info(
            f"Feed loading complete: {added_count} added, {updated_count} updated, {total_count} total active"
        )
    ...
    def get_active_feeds(self) -> list[RSSFeed]:
        ...
        with get_db() as db:
            return db.query(RSSFeed).filter(RSSFeed.is_active.is_(True)).all()
    ...
    def deactivate_failed_feeds(self, error_threshold: int = 5) -> int:
        ...
        with get_db() as db:
            failed_feeds = (
                db.query(RSSFeed)
                .filter(RSSFeed.error_count >= error_threshold, RSSFeed.is_active.is_(True))
                .all()
            )
            ...
            for feed in failed_feeds:
                ...
                deactivated_count += 1
                logger.warning(
                    f"Deactivated feed due to errors: {feed.name} ({feed.error_count} errors)"
                )
    ...
    def get_feed_statistics(self) -> dict:
        ...
        with get_db() as db:
            ...
            active_feeds = db.query(RSSFeed).filter(RSSFeed.is_active.is_(True)).count()
            failed_feeds = db.query(RSSFeed).filter(RSSFeed.error_count > 0).count()

            # Get category breakdown
            categories = {}
            for feed in db.query(RSSFeed).filter(RSSFeed.is_active.is_(True)).all():
