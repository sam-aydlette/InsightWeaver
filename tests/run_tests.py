...
def main():
    ...
    print(f"{Fore.YELLOW}4. Run All Tests")
    print()
    ...
    choice = input("Select test to run (1-4): ").strip() or "3"

    # Set test mode for agent tests
    if choice in ["2", "3", "4"]:
        print()
    ...
    if choice == "1":
        ...
        try:
            print(f"\n{Fore.CYAN}[1/3] Phase 2 Checkpoint Verification...")
            from test_phase2_checkpoint import main as phase2_main
            asyncio.run(phase2_main())
        ...
        try:
            print(f"\n{Fore.CYAN}[2/3] Prioritization Agent Test...")
            from test_prioritization_agent import main as prioritization_main
            asyncio.run(prioritization_main())
        ...
        try:
            print(f"\n{Fore.CYAN}[3/3] Trend Analysis Agent Test...")
            from test_trend_agent import test_trend_analysis
            result = asyncio.run(test_trend_analysis())
        ...
        print(f"\n{Fore.CYAN}{'='*50}")
        if all_passed:
            print(f"{Fore.GREEN}🎉 ALL TESTS PASSED - System ready for deployment")
        else:
