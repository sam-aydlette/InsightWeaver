"""
Tests for running coverage probes against the corpus.
...
from src.matching.coverage_probe import (
    STATUS_INCONCLUSIVE,
    STATUS_MATCHED,
    STATUS_UNMATCHED,
    compile_probe,
...
from src.matching.terms import CoverageProbe
...
def run(session, probes, feed_urls=(FEED_URL,)):
    """
    Run a probe set against an explicit feed selection.

    Before task 012 this built a ``BeatConfig`` and let ``run_coverage_probes``
    resolve its feeds. Beats are gone and the scope is now passed directly, so
    the helper is the call itself.
    """
    return run_coverage_probes(session, tuple(probes), list(feed_urls), subject="test-subject")
...
class TestVerdicts:
    ...
    def test_a_match_on_another_beats_feed_does_not_count_but_is_reported(self, session, corpus):
        ...
        assert result.elsewhere[0].in_selected_sources is False
...
class TestFeedScope:
    def test_the_caller_names_the_scope_and_the_report_echoes_it(self, session, corpus):
        """
        The feed selection is an argument, not something the matcher resolves.

        Until task 012 this took a ``BeatConfig`` and called ``resolve_feeds()``
        when ``feed_urls`` was omitted, which put beat-file loading inside the
        matcher. The scope is now always the caller's, and ``feed_names`` reports
        back the feeds that selection actually resolved to in the corpus.
        """
        in_beat, _ = corpus
        add_article(
            session, in_beat, "NIST publishes vulnerability guidance", datetime(2026, 8, 25)
        ...
        report = run(session, [probe(any_of=(("vulnerabilit*",),))])

        assert report.feed_names == (FEED_NAME,)
        assert report.results[0].status == STATUS_MATCHED

    def test_an_empty_scope_answers_nothing_rather_than_passing(self, session, corpus):
        """No feeds searched is INCONCLUSIVE, never a clean sweep."""
        in_beat, _ = corpus
        add_article(
            session, in_beat, "NIST publishes vulnerability guidance", datetime(2026, 8, 25)
        )

        report = run(session, [probe()], feed_urls=())

        assert report.feed_names == ()
        assert report.results[0].status == STATUS_INCONCLUSIVE

    def test_the_subject_label_is_carried_through_untouched(self, session, corpus):
        report = run(session, [probe()])
