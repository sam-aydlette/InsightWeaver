...
LANDMINES: **Every feature that makes this more engaging moves it back toward being a briefing, which is the product being replaced.** The pressure will not arrive as "let's make it addictive"; it will arrive as "it would be useful to also see...". The acceptance criterion "shows nothing the email does not" is the line, and it is written that way so a proposed addition has to justify itself against the email first. **Auth must fail closed and this is not theoretical**: `silk-reeling-mirror` shipped fail-open under Basic Auth, then **re-implemented auth entirely with Cognito and preserved the same hole**, because a local-dev passthrough reads as a feature rather than as a config typo away from open. Both were fixed on 2026-08-31. A rewrite is not a fix; verify this gate behaviourally with the config absent. **The snapshot contains Position-derived data** -- decisions, deadlines, beliefs -- which is more sensitive than anything currently on samaydlette.com. The S3 object must not be public-read, and the bucket must not be the site's public bucket. `main` is protected -- open a PR.
---
Written 2026-08-31.
